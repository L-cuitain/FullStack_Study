...
```js
//window.onload必须等到页面所有元素加载完毕后才能执行  $(document).ready() 在Dom结构绘制完毕后就可执行 不必等到加载完毕
//window.onload不能同时编写多个  如果有多个方法 只会执行一个  $(document).ready()可以同时编写多个  并且都能执行
//window.onload没有简化写法  $(document).ready() 可以简写成$(function(){})
```
...
```js
    //添加类名  addClass()
    $('ul>li:nth-child(1)').addClass('first');
    
    //删除类名  removeClass()
    $('ul>li:nth-child(1)').removeClass();
    //切换类名  toggleClass()
    $('ul>li:nth-child(2)').toggleClass('second');
    //是否包含类名  hasClass()
    console.log($('ul>li:nth-child(3)').hasClass('third'));
```
