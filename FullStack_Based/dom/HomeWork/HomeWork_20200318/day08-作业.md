...
```js
//事件委托指将子元素需要响应的事件 交给父元素进行监听
//实现原理: 事件委托利用事件冒泡实现 子元素响应事件 都会冒泡到最上层 委托父级元素代执行事件
//应用场景: 当有多个子元素需要进行交互时应用  可以减少次数和程序内存的节省 
```
第2题. 常用的键盘事件有哪些？作用分别是什么？

```js
//1. e.keydown 键盘按键按下时触发
//2. e.keypress 键盘按键按下的时候触发  不能识别ctrl shift 左右箭头
//3. e.keyup 键盘按键弹起时触发
```
第3题.  鼠标事件对象的属性e.pageX和e.clinetX的区别是什么？

```js
//e.pageX 以网页整个页面为基准  显示当前的坐标
//e.clientX 以网页可视化区域为基准  显示当前的坐标
```
