...
1. 在jQuery中，下列选项中(  A )是属性选择器
   ...
   A、$("input[type='button']")  //属性选择器格式: $("[attribute|='value']")

   B、$(":input")   //过滤选择器  标志: 以':'开头

   C、$("#btn")     //id选择器  标志: 以'#'开头

   D、$("div>span")   //子元素选择器   标志: '>'
...
2.    在jQuery中，下面选项中(  C  )能获取焦点 
       ...
       B、select( )   //当textarea或文本类型input元素文本被选中时 会触发该事件

       C、focus( )  //获取焦点事件

       D、onfocus( )  //不属于jquery写法
...
下列能输出"1"的jQuery代码是(  C  )
  ...
  A、alert($(".top1").html( );     //获取不到  没有class为top的元素

  B、alert($("[name='header1]'").html( ));    //属性选择器格式错误

  C、alert($("[name='header1']").html( ));

  D、alert($('#header1').html( ));    //获取不到  没有id为header1的元素
...
则设置选中状态为“女”，正确jQuery实现代码为(  C  )
   ...
   A、$("input:radio").get(0).prop("checked",true);    //会报错 is not a function


   B、$("input[type=radio]")[1].prop("checked",true);   //会报错  Cannot read property 'prop' of undefined


   C、$("input[type='radio']").eq(1).prop("checked",true);


   D、$("sex:radio").eq(1).prop("checked",true)  //元素获取错误
...
​       C、jquery的浏览器兼容很好，能兼容所有的浏览器      //根据浏览器的不同 加载对应的jquery版本
...
​      B、localStorage.setItem("key","value"),该方法用于存储名字为key的一个值value，但是不能更新value    //可以替换value值
...
//本地储存作用:
//能保存更大的数据 并且不手动清除  数据就不会过期  保存的数据不会发给服务器 避免宽带浪费
```
