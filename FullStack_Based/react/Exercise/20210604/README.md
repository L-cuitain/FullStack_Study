...
## 为什么要学习Redux
```
1.React和Vue区别之一: Vue是双向 React是单向

2.React是一个用于构建用户界面的JavaScript库 , 并不是Web应用的完整解决方案

3.当应用程序复杂时 如果想很好的管理 , 维护 , 追踪 , 控制状态(数据)时 需要使用Redux(类似Vuex)
```
...
1.通过store保存数据
2.通过action来修改数据
3.通过reducer将store和action串联 reducer(state=initialState){}
                      -------------
          --------->  | Component |  ---------
         |            -------------           |
         |                                    ⬇
...
### 单一数据源
```
1.整个应用程序的state只能储存在一个store中
2.Redux并没有强制不能创建多个Store , 但那样做并不利于数据的维护
3.单一的数据源可以让整个应用程序的state变得方便维护 , 追踪 , 修改
```

### State是只读的
```
1.唯一修改State的方法一定是触发action
2.确保视频或网络请求都不能直接修改State
3. 保证所有的修改都被集中化处理 , 并按照严格的顺序执行
```
