...
### useEffect 作用
```js
//1.通过使用这个Hook 可以告诉React组件需要在渲染后执行某些操作.React会保存你传递的函数,并在执行DOM更新后调用它

//2.将useEffect放在组件内部可以在Effect中直接访问变量

//3.默认情况下 , useEffect在第一次渲染之后 和 每次更新之后都会执行
```
...
### useEffect 清除作用
```js
//1.每个Effect都可以返回一个清除函数

//2.React会在组件卸载时执行清除操作,Effect在每次渲染时都会执行

//3.effect返回的函数可以返回一个箭头函数或其他命名函数
```
