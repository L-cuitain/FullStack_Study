...
5. 下列关于scrollHeight和scrollWidth说法错误的是(A)
...
```js
//client系列
//clientWidth  作用: 返回元素内部的宽度  包含内边距  不包括水平滚动条 边框 和 外边距
// 计算: clientWidth = width + padding - 水平滚动条宽度
//clientHeight  作用: 返回元素内部的高度  包含内边距  不包括水平滚动条 边框 和 外边距
// 计算: clientWidth = width + padding - 水平滚动条宽度
//clientTop  作用:获取元素顶部边框的宽度  不包括顶部外边距和内边距
//clientLeft  作用: 或i去元素左边的宽度   不包括顶部外边距和内边距
```
第2题. scroll系列包含哪些属性，分别有什么作用？
```js
//scroll系列
//scrollWidth  作用:获取指定元素的真实宽度
//scrollHeight  作用:获取指定元素的真实高度
//scrollTop   作用: 获取一个元素距离它容器顶部的像素距离
//scrollLeft   作用: 获取一个元素距离它容器左部的像素距离
```
