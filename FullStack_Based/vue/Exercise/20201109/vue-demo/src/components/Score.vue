<template>
  <div class="score_container">
    ...
    <div class="share_button" @click="againTime">再来一次</div>
...
export default {
  ...
  setup() {
    ...
    //创建变量 保存分数
    let score = computed(() => store.state.totalScore);
    ...
    function getTip(){
        if(score.value <= 20){
            tip.value = scoreTipsArr[0];
            return;
        }
        if(score.value <= 40){
            tip.value = scoreTipsArr[1];
            return;
        }
        if(score.value <= 60){
            tip.value = scoreTipsArr[2];
            return;
        }
        if(score.value <= 80){
            tip.value = scoreTipsArr[3];
            return;
        }
        if(score.value <= 100){
            tip.value = scoreTipsArr[4];
            return;
        }
    }
